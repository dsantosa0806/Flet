# ==========================================================
# ABA ADMIN - SIOR REGISTRO DE PAGAMENTO
# ==========================================================
import os
import re
import threading
import traceback
from datetime import datetime

import pandas as pd

import config
from selenium.webdriver.common.by import By

from navegador.sior_selenium_execution import (
    iniciar_sessao_sior,
    encerrar_navegador_sior,
    safe_get,
    sincronizar_cookies_navegador_para_session,
)

from requests_data.requisicoes_sior_registro_pagamento import (
    URL_REGISTRO_PAGAMENTO_PAGE,
    LIMITE_REGISTROS_POR_EXECUCAO,
    COLUNAS_LOG,
    executar_registros_pagamento,
    preparar_headers_registro_pagamento,
    inicializar_tela_registro_pagamento,
)

from utils.open_dir_downloads import abrir_pasta_exportacao
from utils.popups import mostrar_alerta


# ==========================================================
# COLUNAS DA PLANILHA
# ==========================================================
COLUNAS_MOLDE = [
    "NumeroAuto",
    "DataPagamento",
    "NumeroDocArrecadacao",
    "Observacao",
]


# ==========================================================
# NORMALIZAÇÃO
# ==========================================================
def normalizar_nome_coluna(coluna: str) -> str:
    return (
        str(coluna or "")
        .strip()
        .upper()
        .replace(" ", "_")
        .replace("-", "_")
        .replace(".", "")
        .replace("Ç", "C")
        .replace("Ã", "A")
        .replace("Á", "A")
        .replace("À", "A")
        .replace("Â", "A")
        .replace("É", "E")
        .replace("Ê", "E")
        .replace("Í", "I")
        .replace("Ó", "O")
        .replace("Ô", "O")
        .replace("Õ", "O")
        .replace("Ú", "U")
    )


def normalizar_auto(valor) -> str:
    if pd.isna(valor):
        return ""

    return str(valor).strip().upper()


def normalizar_doc_arrecadacao(valor) -> str:
    if pd.isna(valor):
        return ""

    texto = str(valor).strip()

    if texto.endswith(".0"):
        texto = texto[:-2]

    return "".join(
        c for c in texto
        if c.isdigit()
    )


def normalizar_observacao(valor) -> str:
    if pd.isna(valor):
        return ""

    return str(valor).strip()


def normalizar_data_pagamento(valor) -> str:
    """
    Normaliza a data para dd/mm/aaaa sem gerar warning do pandas.

    Trata:
    - datetime vindo do Excel
    - string no formato yyyy-mm-dd
    - string no formato yyyy-mm-dd HH:MM:SS
    - string no formato dd/mm/aaaa
    """
    if pd.isna(valor) or str(valor).strip() == "":
        return ""

    # Se já veio como datetime/date do Excel
    if hasattr(valor, "strftime"):
        try:
            return valor.strftime("%d/%m/%Y")
        except Exception:
            pass

    texto = str(valor).strip()

    # Caso venha do Excel como 2026-06-18 00:00:00
    if re.match(r"^\d{4}-\d{2}-\d{2}", texto):
        try:
            data = pd.to_datetime(
                texto,
                format="%Y-%m-%d %H:%M:%S",
                errors="coerce",
            )

            if pd.isna(data):
                data = pd.to_datetime(
                    texto[:10],
                    format="%Y-%m-%d",
                    errors="raise",
                )

            return data.strftime("%d/%m/%Y")

        except Exception:
            pass

    # Caso venha como dd/mm/aaaa
    try:
        data = pd.to_datetime(
            texto,
            format="%d/%m/%Y",
            errors="raise",
        )

        return data.strftime("%d/%m/%Y")

    except Exception:
        return texto


# ==========================================================
# VALIDAÇÃO
# ==========================================================
def validar_df_registro_pagamento(
    df: pd.DataFrame,
):
    erros = []

    if df.empty:
        erros.append(
            "A planilha selecionada está vazia."
        )
        return erros

    if len(df) > LIMITE_REGISTROS_POR_EXECUCAO:
        erros.append(
            f"Limite máximo de {LIMITE_REGISTROS_POR_EXECUCAO} registros por execução."
        )

    autos = df["NumeroAuto"].tolist()

    if len(set(autos)) < len(autos):
        duplicados = sorted(
            {
                c
                for c in autos
                if autos.count(c) > 1
            }
        )

        erros.append(
            "Existem Número de AITs duplicados: "
            + ", ".join(duplicados[:20])
        )

    for idx, row in df.iterrows():
        linha = idx + 2

        numero_auto = str(
            row.get("NumeroAuto", "")
        ).strip()

        data_pagamento = str(
            row.get("DataPagamento", "")
        ).strip()

        numero_doc = str(
            row.get("NumeroDocArrecadacao", "")
        ).strip()

        observacao = str(
            row.get("Observacao", "")
        ).strip()

        # ==================================================
        # NumeroAuto obrigatório
        # Formato: letra + 9 dígitos
        # ==================================================
        if not numero_auto:
            erros.append(
                f"Linha {linha}: NumeroAuto é obrigatório."
            )

        elif not re.match(r"^[A-Za-z][0-9]{9}$", numero_auto):
            erros.append(
                f"Linha {linha}: NumeroAuto inválido ({numero_auto}). "
                "Use o formato Letra + 9 dígitos. Ex: B000000000."
            )

        # ==================================================
        # DataPagamento obrigatória
        # Formato: dd/mm/aaaa
        # ==================================================
        if not data_pagamento:
            erros.append(
                f"Linha {linha}: DataPagamento é obrigatória."
            )

        else:
            try:
                pd.to_datetime(
                    data_pagamento,
                    dayfirst=True,
                    errors="raise",
                )
            except Exception:
                erros.append(
                    f"Linha {linha}: DataPagamento inválida ({data_pagamento}). "
                    "Use o formato dd/mm/aaaa."
                )

        # ==================================================
        # NumeroDocArrecadacao obrigatório
        # Somente dígitos, máximo 17
        # ==================================================
        if not numero_doc:
            erros.append(
                f"Linha {linha}: NumeroDocArrecadacao é obrigatório."
            )

        elif not numero_doc.isdigit():
            erros.append(
                f"Linha {linha}: NumeroDocArrecadacao deve conter somente dígitos."
            )

        elif len(numero_doc) > 17:
            erros.append(
                f"Linha {linha}: NumeroDocArrecadacao deve possuir no máximo 17 dígitos."
            )

        # ==================================================
        # Observacao opcional
        # Máximo 500
        # ==================================================
        if len(observacao) > 500:
            erros.append(
                f"Linha {linha}: Observacao deve possuir no máximo 500 caracteres."
            )

    return erros


def ler_planilha_molde(
    caminho_arquivo: str,
) -> pd.DataFrame:
    if not caminho_arquivo or not os.path.exists(caminho_arquivo):
        raise FileNotFoundError(
            "Selecione uma planilha válida para execução."
        )

    df_original = pd.read_excel(
        caminho_arquivo,
        dtype=str,
    )

    if df_original.empty:
        raise ValueError(
            "A planilha selecionada está vazia."
        )

    mapa_colunas = {
        normalizar_nome_coluna(c): c
        for c in df_original.columns
    }

    # Aceita nomes antigos também: Auto e Data
    mapa_alias = {
        "NUMEROAUTO": ["NUMEROAUTO", "AUTO", "NUMERO_AUTO"],
        "DATAPAGAMENTO": ["DATAPAGAMENTO", "DATA", "DATA_PAGAMENTO"],
        "NUMERODOCARRECADACAO": [
            "NUMERODOCARRECADACAO",
            "NUMERO_DOC_ARRECADACAO",
            "DOCUMENTO_ARRECADACAO",
            "DOC_ARRECADACAO",
        ],
        "OBSERVACAO": ["OBSERVACAO", "OBSERVAÇÃO"],
    }

    colunas_resolvidas = {}

    for coluna_padrao, aliases in mapa_alias.items():
        for alias in aliases:
            alias_norm = normalizar_nome_coluna(
                alias
            )

            if alias_norm in mapa_colunas:
                colunas_resolvidas[
                    coluna_padrao
                ] = mapa_colunas[
                    alias_norm
                ]
                break

    obrigatorias = [
        "NUMEROAUTO",
        "DATAPAGAMENTO",
        "NUMERODOCARRECADACAO",
    ]

    faltantes = [
        c
        for c in obrigatorias
        if c not in colunas_resolvidas
    ]

    if faltantes:
        raise ValueError(
            "A planilha deve conter as colunas obrigatórias: "
            "NumeroAuto, DataPagamento e NumeroDocArrecadacao."
        )

    if "OBSERVACAO" not in colunas_resolvidas:
        df_original["Observacao"] = ""
        colunas_resolvidas["OBSERVACAO"] = "Observacao"

    df = df_original[
        [
            colunas_resolvidas["NUMEROAUTO"],
            colunas_resolvidas["DATAPAGAMENTO"],
            colunas_resolvidas["NUMERODOCARRECADACAO"],
            colunas_resolvidas["OBSERVACAO"],
        ]
    ].copy()

    df.columns = COLUNAS_MOLDE

    df["NumeroAuto"] = df["NumeroAuto"].apply(
        normalizar_auto
    )

    df["DataPagamento"] = df["DataPagamento"].apply(
        normalizar_data_pagamento
    )

    df["NumeroDocArrecadacao"] = df["NumeroDocArrecadacao"].apply(
        normalizar_doc_arrecadacao
    )

    df["Observacao"] = df["Observacao"].apply(
        normalizar_observacao
    )

    df = df[
        df["NumeroAuto"] != ""
    ].copy()

    erros = validar_df_registro_pagamento(
        df
    )

    if erros:
        raise ValueError(
            "\n".join(erros[:80])
        )

    return df


def gerar_planilha_molde(
    caminho_saida: str,
) -> None:
    df = pd.DataFrame(
        [
            {
                "NumeroAuto": "B000000000",
                "DataPagamento": datetime.now().strftime("%d/%m/%Y"),
                "NumeroDocArrecadacao": "00000000000000000",
                "Observacao": "Registro de pagamento realizado conforme comprovação no SAPIENS Dívida.",
            },
            {
                "NumeroAuto": "S014267241",
                "DataPagamento": datetime.now().strftime("%d/%m/%Y"),
                "NumeroDocArrecadacao": "12345678901234567",
                "Observacao": "",
            },
        ]
    )

    with pd.ExcelWriter(
        caminho_saida,
        engine="openpyxl",
    ) as writer:
        df.to_excel(
            writer,
            sheet_name="Molde",
            index=False,
        )

        ws = writer.sheets[
            "Molde"
        ]

        ws.column_dimensions["A"].width = 18
        ws.column_dimensions["B"].width = 18
        ws.column_dimensions["C"].width = 28
        ws.column_dimensions["D"].width = 80

        for cell in ws[1]:
            cell.style = "Headline 3"

        df_regras = pd.DataFrame(
            [
                {
                    "Dado": "NumeroAuto",
                    "Tipo": "Texto",
                    "Tamanho": "10",
                    "Formato": "B000000000",
                    "Obrigatorio": "Sim",
                },
                {
                    "Dado": "DataPagamento",
                    "Tipo": "Data",
                    "Tamanho": "",
                    "Formato": "dd/mm/aaaa",
                    "Obrigatorio": "Sim",
                },
                {
                    "Dado": "NumeroDocArrecadacao",
                    "Tipo": "Texto somente dígitos",
                    "Tamanho": "máximo 17",
                    "Formato": "00000000000000000",
                    "Obrigatorio": "Sim",
                },
                {
                    "Dado": "Observacao",
                    "Tipo": "Texto",
                    "Tamanho": "500",
                    "Formato": "",
                    "Obrigatorio": "Não",
                },
            ]
        )

        df_regras.to_excel(
            writer,
            sheet_name="Leia-me",
            index=False,
        )

        ws2 = writer.sheets[
            "Leia-me"
        ]

        ws2.column_dimensions["A"].width = 28
        ws2.column_dimensions["B"].width = 28
        ws2.column_dimensions["C"].width = 18
        ws2.column_dimensions["D"].width = 28
        ws2.column_dimensions["E"].width = 15

        for cell in ws2[1]:
            cell.style = "Headline 3"


def exportar_logs_excel(
    caminho_saida: str,
    df_molde: pd.DataFrame,
    df_logs: pd.DataFrame,
):
    with pd.ExcelWriter(
        caminho_saida,
        engine="openpyxl",
    ) as writer:

        df_logs.to_excel(
            writer,
            sheet_name="Logs",
            index=False,
        )

        df_molde.to_excel(
            writer,
            sheet_name="Molde Processado",
            index=False,
        )

        if not df_logs.empty and "Status" in df_logs.columns:
            resumo = (
                df_logs.groupby("Status")
                .size()
                .reset_index(name="Quantidade")
                .sort_values("Status")
            )
        else:
            resumo = pd.DataFrame(
                columns=[
                    "Status",
                    "Quantidade",
                ]
            )

        resumo.to_excel(
            writer,
            sheet_name="Resumo",
            index=False,
        )

        for sheet in writer.sheets.values():
            for column_cells in sheet.columns:
                max_length = 0
                column_letter = column_cells[0].column_letter

                for cell in column_cells:
                    try:
                        max_length = max(
                            max_length,
                            len(str(cell.value or "")),
                        )
                    except Exception:
                        pass

                sheet.column_dimensions[
                    column_letter
                ].width = min(
                    max_length + 2,
                    80,
                )


# ==========================================================
# ABA
# ==========================================================
def aba_admin_sior_registro_pagamento(
    ft,
    DEFAULT_FONT_SIZE,
    HEADING_FONT_SIZE,
    page,
    bloquear,
    desbloquear,
):
    alerta_dialogo = ft.AlertDialog(
        modal=True,
        title=ft.Text(""),
        content=ft.Text(""),
        actions=[],
        open=False,
    )

    page.dialog = alerta_dialogo

    pasta_base_saida = getattr(
        config,
        "PASTA_EXPORT_ADMIN",
        r"C:\Downloads",
    )

    os.makedirs(
        pasta_base_saida,
        exist_ok=True,
    )

    estado = {
        "arquivo_molde": None,
        "df_molde": pd.DataFrame(
            columns=COLUNAS_MOLDE
        ),
        "df_logs": pd.DataFrame(
            columns=COLUNAS_LOG
        ),
        "caminho_saida": None,
    }

    # ======================================================
    # CONTROLES
    # ======================================================
    txt_arquivo = ft.Text(
        "Nenhuma planilha selecionada.",
        size=DEFAULT_FONT_SIZE,
        color=ft.Colors.GREY_600,
        selectable=True,
    )

    txt_saida = ft.Text(
        f"📁 Saída padrão: {pasta_base_saida}",
        size=DEFAULT_FONT_SIZE,
        color=ft.Colors.GREY_600,
        selectable=True,
    )

    btn_baixar_molde = ft.ElevatedButton(
        "Gerar planilha molde",
        icon=ft.Icons.DOWNLOAD,
    )

    btn_selecionar = ft.ElevatedButton(
        "Selecionar planilha",
        icon=ft.Icons.UPLOAD_FILE,
    )

    btn_executar = ft.ElevatedButton(
        "Iniciar Registro Pagamento",
        icon=ft.Icons.PLAY_ARROW,
        bgcolor="green",
        color="white",
        disabled=True,
    )

    btn_exportar_logs = ft.ElevatedButton(
        "Abrir XLSX de logs",
        icon=ft.Icons.TABLE_VIEW,
        visible=False,
    )

    btn_limpar_logs = ft.ElevatedButton(
        "Limpar logs",
        icon=ft.Icons.CLEANING_SERVICES,
    )

    progress = ft.ProgressBar(
        width=500,
        visible=False,
    )

    status = ft.Text(
        "",
        size=DEFAULT_FONT_SIZE,
        color="blue",
        visible=False,
    )

    log_execucao = ft.TextField(
        label="📝 Logs do Registro de Pagamento",
        multiline=True,
        read_only=True,
        expand=True,
        min_lines=16,
        max_lines=16,
        label_style=ft.TextStyle(
            size=DEFAULT_FONT_SIZE
        ),
        text_style=ft.TextStyle(
            size=DEFAULT_FONT_SIZE
        ),
    )

    resumo_planilha = ft.Text(
        "",
        size=DEFAULT_FONT_SIZE,
        visible=False,
        selectable=True,
    )

    arquivos_gerados = ft.Column(
        controls=[],
        spacing=5,
        visible=False,
    )

    tabela_preview = ft.DataTable(
        columns=[
            ft.DataColumn(
                ft.Text(
                    "NumeroAuto",
                    size=DEFAULT_FONT_SIZE,
                )
            ),
            ft.DataColumn(
                ft.Text(
                    "DataPagamento",
                    size=DEFAULT_FONT_SIZE,
                )
            ),
            ft.DataColumn(
                ft.Text(
                    "NumeroDocArrecadacao",
                    size=DEFAULT_FONT_SIZE,
                )
            ),
            ft.DataColumn(
                ft.Text(
                    "Observacao",
                    size=DEFAULT_FONT_SIZE,
                )
            ),
        ],
        rows=[],
        visible=False,
        data_text_style=ft.TextStyle(
            size=DEFAULT_FONT_SIZE
        ),
    )

    container_preview = ft.Container(
        content=tabela_preview,
        height=240,
        padding=10,
        border_radius=10,
        border=ft.border.all(
            1,
            ft.Colors.GREY_600,
        ),
        bgcolor=ft.Colors.with_opacity(
            0.05,
            ft.Colors.ON_SURFACE,
        ),
        visible=False,
    )

    file_picker = ft.FilePicker()
    page.overlay.append(file_picker)

    # ======================================================
    # HELPERS UI
    # ======================================================
    def adicionar_log(
        mensagem: str,
    ):
        try:
            horario = datetime.now().strftime(
                "%H:%M:%S"
            )

            log_execucao.value += (
                f"[{horario}] {mensagem}\n"
            )

            status.value = mensagem
            status.visible = True

            page.update()

        except Exception:
            pass

    def registrar_arquivo(
        label: str,
        caminho: str,
    ):
        arquivos_gerados.controls.append(
            ft.Text(
                f"✅ {label}: {caminho}",
                size=DEFAULT_FONT_SIZE,
                selectable=True,
            )
        )

        arquivos_gerados.visible = True

    def atualizar_preview():
        tabela_preview.rows.clear()

        df = estado[
            "df_molde"
        ]

        for _, row in df.head(5).iterrows():
            tabela_preview.rows.append(
                ft.DataRow(
                    cells=[
                        ft.DataCell(
                            ft.Text(
                                str(row.get("NumeroAuto", "")),
                                size=DEFAULT_FONT_SIZE,
                            )
                        ),
                        ft.DataCell(
                            ft.Text(
                                str(row.get("DataPagamento", "")),
                                size=DEFAULT_FONT_SIZE,
                            )
                        ),
                        ft.DataCell(
                            ft.Text(
                                str(row.get("NumeroDocArrecadacao", "")),
                                size=DEFAULT_FONT_SIZE,
                            )
                        ),
                        ft.DataCell(
                            ft.Text(
                                str(row.get("Observacao", ""))[:120],
                                size=DEFAULT_FONT_SIZE,
                            )
                        ),
                    ]
                )
            )

        total = len(
            df
        )

        resumo_planilha.value = (
            f"📄 Planilha carregada: {total} registro(s) para pagamento."
        )

        resumo_planilha.visible = True
        tabela_preview.visible = total > 0
        container_preview.visible = total > 0
        btn_executar.disabled = total == 0

        page.update()

    def limpar_logs(e=None):
        log_execucao.value = ""

        arquivos_gerados.controls.clear()
        arquivos_gerados.visible = False

        status.value = ""

        btn_exportar_logs.visible = False

        estado["df_logs"] = pd.DataFrame(
            columns=COLUNAS_LOG
        )

        estado["caminho_saida"] = None

        page.update()

    # ======================================================
    # AÇÕES PLANILHA
    # ======================================================
    def baixar_molde(e):
        try:
            ts = datetime.now().strftime(
                "%Y-%m-%d_%H-%M-%S"
            )

            caminho = os.path.join(
                pasta_base_saida,
                f"molde_sior_registro_pagamento_{ts}.xlsx",
            )

            gerar_planilha_molde(
                caminho
            )

            mostrar_alerta(
                ft,
                page,
                "Molde gerado",
                f"✅ Planilha molde de registro de pagamento gerada em:\n{caminho}",
                tipo="success",
            )

            abrir_pasta_exportacao(
                caminho
            )

        except Exception as ex:
            mostrar_alerta(
                ft,
                page,
                "Erro ao gerar molde",
                str(ex),
                tipo="error",
            )

        finally:
            page.update()

    def selecionar_planilha(e):
        file_picker.pick_files(
            allow_multiple=False,
            allowed_extensions=[
                "xlsx",
                "xls",
            ],
        )

    def on_file_picked(e):
        try:
            if not e.files:
                return

            caminho = e.files[0].path

            estado[
                "arquivo_molde"
            ] = caminho

            txt_arquivo.value = (
                f"📄 Planilha selecionada: {caminho}"
            )

            df = ler_planilha_molde(
                caminho
            )

            estado[
                "df_molde"
            ] = df

            atualizar_preview()

            mostrar_alerta(
                ft,
                page,
                "Planilha carregada",
                f"✅ {len(df)} registro(s) carregado(s) para pagamento.",
                tipo="success",
            )

        except Exception as ex:
            estado["df_molde"] = pd.DataFrame(
                columns=COLUNAS_MOLDE
            )

            btn_executar.disabled = True
            container_preview.visible = False
            resumo_planilha.visible = False

            mostrar_alerta(
                ft,
                page,
                "Validação da planilha",
                str(ex),
                tipo="error",
            )

        finally:
            page.update()

    def abrir_logs(e=None):
        caminho = estado.get(
            "caminho_saida"
        )

        if caminho and os.path.exists(caminho):
            abrir_pasta_exportacao(
                caminho
            )
        else:
            mostrar_alerta(
                ft,
                page,
                "Arquivo não encontrado",
                "O XLSX de logs ainda não foi gerado ou não está disponível.",
                tipo="warning",
            )

            page.update()

    # ======================================================
    # EXECUÇÃO
    # ======================================================
    def executar_registro_pagamento(e):
        try:
            df_molde = estado[
                "df_molde"
            ].copy()

            if df_molde.empty:
                raise ValueError(
                    "Selecione e valide uma planilha antes de iniciar."
                )

            if estado.get("arquivo_molde"):
                df_molde = ler_planilha_molde(
                    estado["arquivo_molde"]
                )

                estado[
                    "df_molde"
                ] = df_molde

        except Exception as ex:
            mostrar_alerta(
                ft,
                page,
                "Validação",
                str(ex),
                tipo="error",
            )

            page.update()
            return

        btn_executar.disabled = True
        btn_executar.text = "Executando..."

        btn_selecionar.disabled = True
        btn_baixar_molde.disabled = True

        progress.visible = True
        progress.value = None

        status.visible = True
        status.value = "Preparando execução..."

        log_execucao.value = ""

        arquivos_gerados.controls.clear()
        arquivos_gerados.visible = False

        btn_exportar_logs.visible = False

        page.update()

        def task():
            navegador = None

            try:
                bloquear()

                ts = datetime.now().strftime(
                    "%Y-%m-%d_%H-%M-%S"
                )

                pasta_saida = os.path.join(
                    pasta_base_saida,
                    f"Registro_Pagamento_SIOR_{ts}",
                )

                os.makedirs(
                    pasta_saida,
                    exist_ok=True,
                )

                adicionar_log(
                    "🔐 Iniciando sessão SIOR..."
                )

                navegador, session = iniciar_sessao_sior(
                    log=adicionar_log
                )

                if navegador is None or session is None:
                    raise RuntimeError(
                        "Não foi possível iniciar a sessão SIOR."
                    )

                adicionar_log(
                    "🌐 Acessando tela interna do SIOR para validar sessão..."
                )

                sucesso_tela = safe_get(
                    navegador=navegador,
                    url=URL_REGISTRO_PAGAMENTO_PAGE,
                    elemento_validacao=(By.TAG_NAME, "body"),
                    tentativas=3,
                    timeout_get=20,
                    timeout_elemento=10,
                    tempo_estabilizacao=1,
                )

                if not sucesso_tela:
                    raise RuntimeError(
                        "Não foi possível acessar uma tela interna do SIOR para validar a sessão."
                    )

                total_cookies = sincronizar_cookies_navegador_para_session(
                    navegador,
                    session,
                )

                preparar_headers_registro_pagamento(
                    session
                )

                adicionar_log(
                    f"🍪 Cookies sincronizados navegador → requests: {total_cookies}."
                )

                inicializar_tela_registro_pagamento(
                    session=session,
                    log=adicionar_log,
                )

                adicionar_log(
                    "✅ Sessão requests validada. Iniciando registros de pagamento."
                )

                adicionar_log(
                    "🚀 Iniciando registros de pagamento..."
                )

                df_logs = executar_registros_pagamento(
                    session=session,
                    df_molde=df_molde,
                    log=adicionar_log,
                    pausa_entre_registros=0.5,
                )

                if df_logs.empty:
                    df_logs = pd.DataFrame(
                        columns=COLUNAS_LOG
                    )

                estado[
                    "df_logs"
                ] = df_logs

                caminho_logs = os.path.join(
                    pasta_saida,
                    f"logs_registro_pagamento_sior_{ts}.xlsx",
                )

                exportar_logs_excel(
                    caminho_logs,
                    df_molde=df_molde,
                    df_logs=df_logs,
                )

                estado[
                    "caminho_saida"
                ] = caminho_logs

                registrar_arquivo(
                    "XLSX de logs",
                    caminho_logs,
                )

                total_sucesso = (
                    int((df_logs["Status"] == "SUCESSO").sum())
                    if "Status" in df_logs
                    else 0
                )

                total_erro = (
                    int((df_logs["Status"] == "ERRO").sum())
                    if "Status" in df_logs
                    else 0
                )

                adicionar_log(
                    f"🎉 Execução finalizada. Sucessos: {total_sucesso} | Erros: {total_erro}."
                )

                btn_exportar_logs.visible = True

                mostrar_alerta(
                    ft,
                    page,
                    "Registro de Pagamento finalizado",
                    (
                        f"✅ Execução concluída.\n"
                        f"Sucessos: {total_sucesso}\n"
                        f"Erros: {total_erro}\n\n"
                        f"Logs em:\n{caminho_logs}"
                    ),
                    tipo="success" if total_erro == 0 else "warning",
                )

                try:
                    abrir_pasta_exportacao(
                        caminho_logs
                    )
                except Exception:
                    pass

            except Exception as ex:
                adicionar_log(
                    f"❌ Erro durante execução: {ex}"
                )

                adicionar_log(
                    traceback.format_exc()
                )

                mostrar_alerta(
                    ft,
                    page,
                    "Erro no Registro de Pagamento",
                    str(ex),
                    tipo="error",
                )

            finally:
                try:
                    if navegador:
                        encerrar_navegador_sior(
                            navegador,
                            log=adicionar_log,
                        )
                except Exception:
                    pass

                btn_executar.disabled = estado[
                    "df_molde"
                ].empty

                btn_executar.text = "Iniciar Registro Pagamento"

                btn_selecionar.disabled = False
                btn_baixar_molde.disabled = False

                progress.visible = False

                desbloquear()

                page.update()

        threading.Thread(
            target=task,
            daemon=True,
        ).start()

    # ======================================================
    # EVENTS
    # ======================================================
    file_picker.on_result = on_file_picked
    btn_baixar_molde.on_click = baixar_molde
    btn_selecionar.on_click = selecionar_planilha
    btn_executar.on_click = executar_registro_pagamento
    btn_exportar_logs.on_click = abrir_logs
    btn_limpar_logs.on_click = limpar_logs

    # ======================================================
    # RETURN
    # ======================================================
    return ft.Column(
        controls=[
            ft.Row(
                [
                    ft.Text(
                        "Admin > SIOR > Registro de Pagamento",
                        size=10,
                        weight="bold",
                    )
                ],
                alignment="center",
            ),

            ft.Divider(),

            ft.Container(
                content=ft.Column(
                    controls=[
                        ft.Text(
                            "⚙️ Registro de Pagamento por Requisição",
                            size=HEADING_FONT_SIZE,
                            weight="bold",
                        ),

                        ft.Text(
                            "Carregue uma planilha com as colunas NumeroAuto, DataPagamento, "
                            "NumeroDocArrecadacao e Observacao. "
                            "A validação é feita antes da execução e o processamento registra "
                            "logs auto a auto, com exportação em XLSX.",
                            size=DEFAULT_FONT_SIZE,
                        ),

                        ft.Row(
                            controls=[
                                btn_baixar_molde,
                                btn_selecionar,
                                btn_executar,
                                btn_exportar_logs,
                                btn_limpar_logs,
                            ],
                            vertical_alignment=ft.CrossAxisAlignment.END,
                            wrap=True,
                        ),

                        txt_arquivo,
                        txt_saida,
                        resumo_planilha,
                    ],
                    spacing=10,
                ),
                padding=15,
                border_radius=10,
                border=ft.border.all(
                    1,
                    ft.Colors.GREY_600,
                ),
                bgcolor=ft.Colors.with_opacity(
                    0.05,
                    ft.Colors.ON_SURFACE,
                ),
            ),

            container_preview,

            status,

            progress,

            ft.Container(
                content=ft.Column(
                    controls=[
                        ft.Text(
                            "📋 Acompanhamento da execução",
                            size=DEFAULT_FONT_SIZE + 1,
                            weight="bold",
                        ),
                        log_execucao,
                    ],
                    spacing=10,
                ),
                padding=10,
                border_radius=10,
                border=ft.border.all(
                    1,
                    ft.Colors.GREY_600,
                ),
                bgcolor=ft.Colors.with_opacity(
                    0.05,
                    ft.Colors.ON_SURFACE,
                ),
            ),

            arquivos_gerados,

            alerta_dialogo,
        ],
        expand=True,
        spacing=10,
        scroll=ft.ScrollMode.AUTO,
    )