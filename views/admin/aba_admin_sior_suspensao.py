# ==========================================================
# ABA ADMIN - SIOR SUSPENSÃO DE COBRANÇA
# ==========================================================
import os
import re
import threading
...
from requests_data.requisicoes_sior_suspensao import (
    URL_CREATE_PAGE,
    LIMITE_AUTOS_POR_REQUISICAO,
    executar_suspensoes_por_motivo,
    preparar_headers_suspensao,
...
TEXTO_PADRAO_OBSERVACAO = (
    "Auto de Infração suspenso automaticamente a partir da captura da informação de "
    f"suspensão registrada no sistema Super Sapiens em {datetime.now().strftime('%d/%m/%Y')}."
)
        ...
        [
            ...
            },
            {
                ...
                "MOTIVO": (
                    ...
                    "Auto de Infração de Trânsito suspenso em razão do "
                    "parcelamento da dívida junto à PFE."
                ),
            },
        ]
    )
...
def aba_admin_sior_suspensao(
    ...
    input_motivo_padrao = ft.TextField(
        label="Motivo padrão da suspensão / observação",
    ...
    def baixar_molde(e):
        try:
            ...
            caminho = os.path.join(
                pasta_base_saida,
                f"molde_sior_suspensao_{ts}.xlsx",
            )
    ...
    def on_file_picked(e):
        try:
            ...
            mostrar_alerta(
                ...
                page,
                "Planilha carregada",
                f"✅ {len(df)} AIT(s) carregado(s) para suspensão.",
                tipo="success",
            )
    ...
    def executar_suspensao(e):
        try:
            ...
            # Isso permite alterar o motivo padrão após selecionar a planilha.
        ...
        def task():
            ...
            try:
                ...
                adicionar_log(
                    "🌐 Acessando tela de Suspensão de Cobrança..."
                )
                ...
                if not sucesso_tela:
                    raise RuntimeError(
                        "Não foi possível acessar a tela de Suspensão de Cobrança do SIOR."
                    )
                ...
                df_logs = executar_suspensoes_por_motivo(
                    session=session,
                    df_molde=df_molde,
                    log=adicionar_log,
                ...
                caminho_logs = os.path.join(
                    pasta_saida,
                    f"logs_suspensao_sior_{ts}.xlsx",
                )
                ...
                btn_executar.text = "Iniciar Suspensão"
    ...
    btn_executar.on_click = executar_suspensao
    ...
    return ft.Column(
        controls=[
            ft.Row(
                [
                    ft.Text(
                        "Admin > SIOR > Suspensão de Cobrança",
                        size=10,
                        weight="bold",
                    )
                ],
            ...
            ft.Container(
                content=ft.Column(
                    controls=[
                        ft.Text(
                            "⚙️ Suspensão de Cobrança por Requisição",
                        ...
                        ft.Text(
                            ...
                            "confirma a suspensão e gera XLSX de logs com sucessos e erros.",
                            size=DEFAULT_FONT_SIZE,
                        ),
